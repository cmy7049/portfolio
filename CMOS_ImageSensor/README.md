...
#### 1. 박막 물성 데이터 직접 확보
DTI 격벽 소재로 검토 중이던 TiO₂의 실측 광학 특성 데이터가 논문만으로는 부족했습니다. 부산에서 서울 SEMICON Korea까지 직접 참관하여 소재 기업 부스 엔지니어로부터 굴절률(n), 소광계수(k) 데이터를 현장에서 확보했습니다.

#### 2. DTI 격벽 소재 및 두께 최적화
격벽 두께를 2nm~600nm까지 50nm 단위로 분할하고, SiO₂·TiO₂·HfO₂·W·BlackDiamond 5개 소재별로 각 30회씩 FDTD 시뮬레이션을 수행하여 포토다이오드 도달 광량을 비교했습니다. 기존 SiO₂ 대비 High-k 소재인 TiO₂가 Crosstalk 억제와 수광율 확보에 가장 효과적인 것을 확인했습니다.

#### 3. ARC 설계
파장 550nm 기준으로 표면 반사 손실을 최소화하기 위해, TiO₂의 굴절률(n=2.435)을 참고하여 최적 ARC 두께(d=56.5nm)를 산출하고 시뮬레이션으로 검증했습니다.
...
ARC 및 최적 DTI 조합을 적용해 수광 효율을 약 13.6% 향상시켰으며, 파라미터 최적화 역량을 인정받아 캡스톤 경진대회 장려상을 수상했습니다.

- QE 0.374 → 0.425 (13.6% 향상)
- 캡스톤디자인 경진대회 장려상 수상 (2024.07.04)
- Semicon Korea에서 High-k 물질 데이터 직접 확보
